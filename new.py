...
opt_disc = optim.Adam(list(D_h.parameters()) + list(D_z.parameters()), lr=0.0002, betas=(0.5, 0.999))
opt_gen = optim.Adam(list(G_z.parameters()) + list(G_h.parameters()), lr=0.0002, betas=(0.5, 0.999))
...
def save_best_checkpoint(epoch, G_loss, D_loss):
    ...
    if G_loss < best_G_loss or D_loss < best_D_loss:
...
for epoch in range(num_epochs):
    ...
    for i, (real_face, real_lego) in progress_bar:
        ...
        G_loss += 20 * (criterion_Cycle(real_face, G_z(G_h(real_face))) + criterion_Cycle(real_lego, G_h(G_z(real_lego))))
...
image_filenames = random.sample(os.listdir("./dataset/human"), 10)
...
for img_name in image_filenames:
    img = Image.open(os.path.join("./dataset/human", img_name)).convert("RGB")
...
plt.tight_layout()
plt.savefig("./output/generated_grid_3.png")
plt.show()
